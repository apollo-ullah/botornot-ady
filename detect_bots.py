...
Usage:
  ...
  Inference:  python detect_bots.py <input.json> <output.txt>
"""
...
def compute_temporal_features(posts):
    ...
    if len(posts) < 2:
        return {
            ...
            'posts_per_hour': 0, 'hour_entropy': 0, 'night_ratio': 0,
...
class BotDetectorEnsemble:
    ...
    def train(self, df, verbose=True):
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"Training {self.lang.upper()} model | Samples: {len(y)} | Bots: {sum(y)} ({100*sum(y)/len(y):.1f}%)")
        ...
        # Optimize threshold
        self.threshold, best_score, results = optimize_threshold(y, all_probs)
        ...
        if verbose:
            ...
            score, tp, fn, fp, tn = compute_score(y, y_pred)
            total_bots = sum(y)
...
def train_pipeline(verbose=True):
    ...
    # Train English model
    if en_dfs:
        en_df = pd.concat(en_dfs, ignore_index=True)
        en_model = BotDetectorEnsemble(lang='en')
        en_model.train(en_df, verbose=verbose)
    ...
    # Train French model
    if fr_dfs:
        fr_df = pd.concat(fr_dfs, ignore_index=True)
        fr_model = BotDetectorEnsemble(lang='fr')
        fr_model.train(fr_df, verbose=verbose)
